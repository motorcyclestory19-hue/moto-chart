...
function submitSignup(e) {
  ...
  const role  = isAdminPhone(phone) ? 'admin' : 'member';

  // 즉시 저장
  users.push({ userId, email, pw, phone, role });
  localStorage.setItem('motoUsers', JSON.stringify(users));

  // 자동 로그인
  currentUser = { userId, email, phone, role };
  if (role === 'admin') {
    isAdminLoggedIn = true;
    showToast(`🎉 가입 완료! ${userId}님, 관리자로 등록되었습니다!`);
    loadHistoryData();
  } else {
    showToast(`🎉 가입 완료! ${userId}님, 환영합니다!`);
  }
